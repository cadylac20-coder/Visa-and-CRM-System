...
def _send_sms(phone: str, message: str, event: str):
    if not TWILIO_SID or not TWILIO_AUTH or not TWILIO_PHONE: return False
    phone = phone.strip().replace(" ", "").replace("-", "")
    if not phone.startswith("+"): phone = "+91" + phone  # assume India if no country code

    url = f"https://api.twilio.com/2010-04-01/Accounts/{TWILIO_SID}/Messages.json"
    try:
        r = requests.post(url, auth=(TWILIO_SID, TWILIO_AUTH), data={"From": TWILIO_PHONE, "To": phone, "Body": message}, timeout=10)
...
def _send_email(to_email: str, subject: str, body_text: str, body_html: str, event: str):
    if not GMAIL_ADDR or not GMAIL_PASS or not to_email: return False
    try:
        msg = MIMEMultipart("alternative")
        msg["Subject"] = subject; msg["From"] = f"Uniglobe MKOV Travel <{GMAIL_ADDR}>"; msg["To"] = to_email
...
def send_checklist(
    ...
    pdf_download_link = f"https://arya-v1-0-0.onrender.com/client/download-checklist/{app_id}"

    wa_message = (
        f"Hi {client_name}! 🙏\n\n"
        f"Your visa application for {destination} ({visa_type.title()}) has been created.\n"
        f"📋 Application ID: {app_id}\n\n"
        f"Download your PDF Document Checklist here:\n{pdf_download_link}\n\n"
    ...
    sms_message = (
        ...
        f"Download PDF checklist: {pdf_download_link} "
        f"Upload at https://arya-v1-0-0.onrender.com/client Call: 8010700700"
    )
      ...
      <div style="background:#f9f9f9;padding:24px;border:1px solid #e0e0e0">
        ...
        <div style="text-align:center;margin:24px 0">
          <a href="{pdf_download_link}" 
             style="background:#00c6b8;color:black;padding:12px 28px;border-radius:6px;text-decoration:none;font-weight:bold;margin-right:10px;">
            Download PDF Checklist ↓
          </a>
        </div>
        <p><strong>Required Documents Quick Look:</strong></p>
    ...
    email_text = (
        ...
        f"Download your PDF Checklist here:\n{pdf_download_link}\n\n"
        f"Documents needed:\n{checklist_text}\n\n"
        f"Upload at: https://arya-v1-0-0.onrender.com/client\n"
    ...
    def _run():
        if "whatsapp" in channels: _send_whatsapp(client_phone, wa_message, "send_checklist")
        if "sms" in channels: _send_sms(client_phone, sms_message, "send_checklist")
        if "email" in channels: _send_email(client_email, f"Visa Application Created — {app_id} — Documents Required", email_text, email_html, "send_checklist")

    _notify_async(_run)

def send_status_update(client_name: str, client_phone: str, client_email: str, app_id: str, new_status: str, note: str = "", channels: list = None):
    if channels is None: channels = ["whatsapp", "email"]
    ...
    wa_message = f"Hello {client_name}! {emoji}\n\nYour visa application status updated.\n📋 {app_id}\nStatus: {new_status.replace('_', ' ').title()}\n\n{message}\n" + (f"\n📝 {note}\n" if note else "") + f"\nView: https://arya-v1-0-0.onrender.com/client\nCall: +91-8010700700"
    sms_message = f"Hi {client_name}! Your visa {app_id} status: {new_status.replace('_', ' ')}. Check https://arya-v1-0-0.onrender.com/client"
    email_html = f'<div style="font-family:Arial;max-width:600px;margin:auto;padding:20px"><div style="background:#0d3055;color:white;padding:24px;border-radius:8px 8px 0 0;text-align:center"><h1 style="margin:0;font-size:24px">{emoji}</h1><h2 style="margin:8px 0;text-transform:uppercase">{new_status.replace("_", " ")}</h2></div><div style="background:#f9f9f9;padding:24px;border:1px solid #e0e0e0"><p>Dear <strong>{client_name}</strong>,</p><p>{message}</p>{"<p style=\'background:#fff3cd;padding:12px;border-radius:6px\'><strong>Note:</strong> " + note + "</p>" if note else ""}<div style="text-align:center;margin:24px 0"><a href="https://arya-v1-0-0.onrender.com/client" style="background:#0d3055;color:white;padding:12px 28px;border-radius:6px;text-decoration:none;font-weight:bold">View Application →</a></div></div></div>'
    email_text = f"Dear {client_name},\n\n{message}\n\n{note}\n\nView at https://arya-v1-0-0.onrender.com/client"

    def _run():
        if "whatsapp" in channels: _send_whatsapp(client_phone, wa_message, "status_update")
        if "sms" in channels: _send_sms(client_phone, sms_message, "status_update")
        if "email" in channels: _send_email(client_email, f"Visa Update — {app_id}", email_text, email_html, "status_update")
    _notify_async(_run)

def send_reminder(client_name: str, client_phone: str, client_email: str, app_id: str, missing_docs: list, channels: list = None):
    if channels is None: channels = ["whatsapp", "email"]
    docs_text = "\n".join(f"  • {doc}" for doc in missing_docs)
    docs_html = "".join(f"<li>{doc}</li>" for doc in missing_docs)

    wa_message = f"Hi {client_name}! 🔔\n\nReminder: We're awaiting documents for {app_id}.\n\nMissing:\n{docs_text}\n\nUpload: https://arya-v1-0-0.onrender.com/client\nCall: +91-8010700700"
    sms_message = f"Hi {client_name}, reminder: upload missing documents for {app_id} at https://arya-v1-0-0.onrender.com/client"
    email_html = f'<div style="font-family:Arial;max-width:600px;margin:auto;padding:20px"><div style="background:#0d3055;color:white;padding:24px;border-radius:8px 8px 0 0"><h1 style="margin:0">Document Reminder</h1></div><div style="background:#f9f9f9;padding:24px;border:1px solid #e0e0e0"><p>Dear {client_name},</p><p>We\'re still waiting for the following documents:</p><ul style="line-height:1.8">{docs_html}</ul><div style="text-align:center;margin:24px 0"><a href="https://arya-v1-0-0.onrender.com/client" style="background:#0d3055;color:white;padding:12px 28px;border-radius:6px;text-decoration:none;font-weight:bold">Upload Now →</a></div></div></div>'
    email_text = f"Dear {client_name},\n\nPlease upload:\n{docs_text}\n\nAt https://arya-v1-0-0.onrender.com/client"

    def _run():
        if "whatsapp" in channels: _send_whatsapp(client_phone, wa_message, "docs_reminder")
